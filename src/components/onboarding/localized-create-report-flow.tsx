...
export function LocalizedCreateReportFlow({ locale, dictionary, initiallyAuthenticated }: { locale: Locale; dictionary: Dictionary; initiallyAuthenticated: boolean }) {
  ...
  async function generate() {
    ...
    try {
      ...
      if (response.status === 401) { setAuthenticated(false); setError(copy.verification.sessionExpired); setLoading(false); return; }
  ...
  const canContinue = step === 1 ? Boolean(rawText && parsed && parsed.messages.length >= 8) : step === 4 ? Boolean(chatName.trim() && participantNamesValid) : true;
