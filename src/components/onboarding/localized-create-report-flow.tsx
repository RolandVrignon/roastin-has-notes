...
export function LocalizedCreateReportFlow({ locale, dictionary }: { locale: Locale; dictionary: Dictionary }) {
  ...
  function useSample() {
    setRawText(sampleChat); setFileName("lisbon-planning-committee.txt"); setChatName("The Lisbon Planning Committee"); setError("");
  }
  ...
  async function generate() {
    ...
    try {
      const response = await fetch("/api/reports/generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ chatName, chatType, context, locale, rawText }) });
  ...
  const canContinue = step !== 3 || Boolean(rawText && parsed && parsed.messages.length >= 8);
    ...
    <main className="min-h-screen bg-[#fffaf0]" lang={locale}>
      ...
      <div className="mx-auto max-w-3xl px-5 pb-16 pt-7 md:pt-10">
        ...
        <section className="mt-10 min-h-[510px]">
          ...
          {step === 4 && <><StepTitle eyebrow={copy.review.eyebrow} text={copy.review.text} title={copy.review.title} /><label className="text-xs font-black uppercase tracking-[.13em] text-[#3b4d5f]">{copy.review.chatName}<input className="mt-2 block w-full rounded-2xl border-2 border-[#112b4d]/20 bg-white px-5 py-4 text-lg font-bold outline-none focus:border-[#e84b20]" maxLength={80} onChange={(event) => setChatName(event.target.value)} value={chatName} /></label><div className="mt-7 space-y-3">{parsed?.participants.map((participant, index) => <div className="flex items-center gap-4 rounded-2xl border border-[#112b4d]/15 bg-white p-4" key={participant.name}><span className={`grid size-10 place-items-center rounded-full font-black ${["bg-[#f6a913]", "bg-[#a9c9a9]", "bg-[#e84b20] text-white"][index % 3]}`}>{participant.name.charAt(0).toUpperCase()}</span><strong>{participant.name}</strong><span className="ml-auto text-sm font-bold text-[#3b4d5f]">{participant.messageCount.toLocaleString(locale)} · {participant.share}%</span></div>)}</div></>}
