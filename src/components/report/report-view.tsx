...
export function FullReport({ report, ui = reportUi(report.locale) }: { report: RoastReport; ui?: ReportUi }) {
  return <>
    ...
    <section className="bg-[#e84b20] px-5 py-20 text-center text-white md:py-28"><Star className="mx-auto text-[#f6a913]" fill="currentColor" size={34} /><span className="mt-5 block text-xs font-black uppercase tracking-[.16em]">{ui.finalKicker}</span><p className="display mx-auto mt-6 max-w-4xl text-4xl font-black leading-[1.2] md:text-6xl">{report.finalVerdict}</p><button className="btn btn-cream mt-10" onClick={() => navigator.share?.({ title: report.title, url: window.location.href })} type="button">{ui.shareGroup} <Share2 size={18} /></button></section>
  </>;
}
